...
class Main extends CI_Controller
{
    ...
    public function ajax_check_member_register()
    {
        ...
        if ($check_member == 1) {  // CHECK PERSON DATA IN ORACLE
            ...
            foreach ($data as $r) {
                array_push($email, "{$r->REG_USERNAME}@rtarf.mi.th");
            }
            $member['valid']    = 'multi-email';
        } else { // NOT FOUND DATA IN ORACLE
            $member['valid']    = false;
        }

        if ($member['valid'] == 'correct') {  // DATA HAS FOUND IN ORACLE
            $num = $this->main_model->check_registered($idp, $round)->num_rows();
            if ($num == 0) {  // CHECK DATA IF NOT REGISTER YET
